...
def sdpa_with_v2t_pos_bias(
    query: torch.Tensor,    # [B,H,Q,D]
    key:   torch.Tensor,    # [B,H,K,D]
    value: torch.Tensor,    # [B,H,K,D]
    noun_idx,               # iterable[int] (텍스트 토큰 인덱스)
    verb_idx,               # iterable[int] (텍스트 토큰 인덱스)
    text_len: int = 226,         
    noun_w: float = +5.0,   # 명사 가중
    verb_w: float = -3.0,   # 동사 가중
    gamma:  float = 1.0,    # 전역 스케일
    normalize_bias: bool = True,  # 평균 0 정규화
    base_attn_mask: torch.Tensor | None = None,  # 기존 마스크(브로드캐스트 가능 모양 권장)
    dropout_p: float = 0.0,
    is_causal: bool = False,
):
    """
    - Concat self-attn에서 Q=K=[text | video].
    - 비디오 쿼리(Q_img) -> 텍스트 키(K_text) 경로만 바이어스 적용.
    - OOM 피하려고 Q를 분할해 SDPA를 2번 호출하고, 키쪽 1D 바이어스만 쓴다.
    """
    assert query.dim() == key.dim() == value.dim() == 4, "Expect [B,H,Q,D]/[B,H,K,D]"
    B, H, Q, D = query.shape
    K = key.size(-2)
    assert K == Q, "Here we assume Q==K (concat self-attn). Adjust if different."

    T = int(text_len)
    T = max(0, min(T, K))   # 안전 클램프

    # 1) 키쪽 1D 바이어스 만들기 (텍스트 구간만 가중치, 비디오는 0)
    bias_1d = torch.zeros(K, device=query.device, dtype=query.dtype)

    if T > 0:
        noun_idx_t = torch.as_tensor(noun_idx, device=query.device)
        verb_idx_t = torch.as_tensor(verb_idx, device=query.device)

        # 텍스트 범위로 제한
        noun_idx_t = noun_idx_t[(noun_idx_t >= 0) & (noun_idx_t < T)]
        verb_idx_t = verb_idx_t[(verb_idx_t >= 0) & (verb_idx_t < T)]

        if noun_idx_t.numel() > 0:
            src_noun = torch.full((noun_idx_t.numel(),), noun_w, device=bias_1d.device, dtype=bias_1d.dtype)
            bias_1d.index_add_(0, noun_idx_t, src_noun)
        if verb_idx_t.numel() > 0:
            src_verb = torch.full((verb_idx_t.numel(),), verb_w, device=bias_1d.device, dtype=bias_1d.dtype)
            bias_1d.index_add_(0, verb_idx_t, src_verb)

        """ normalization 잠시 제거 """
        # if normalize_bias and T > 1:
        #     # 텍스트 구간 평균 0 정규화 후 γ 스케일
        #     mean = bias_1d[:T].mean()
        #     bias_1d[:T] = (bias_1d[:T] - mean) * gamma
        # else:
        #     bias_1d[:T] = bias_1d[:T] * gamma

    # 2) 브로드캐스트용 모양으로 변환: [1,1,1,K]
    bias_mask = bias_1d.view(1, 1, 1, K)

    # 3) 쿼리를 텍스트/비디오로 분할
    q_text  = query[..., :T, :]          # [B,H,T,D] (T가 0일 수도)
    q_video = query[..., T:, :]          # [B,H,Q-T,D]

    # 4) 텍스트 쿼리 → 기본 마스크만(있다면) 적용, 바이어스는 없음
    if q_text.size(-2) > 0:
        attn_mask_text = base_attn_mask  # 그대로 전달 (브로드캐스트 가능한 모양을 권장)
        ...
        out_text = q_text  # 빈 텐서

    # 5) 비디오 쿼리 → 텍스트 키 바이어스를 attn_mask로 전달
    #    기존 마스크가 작게 브로드캐스트 가능하면 합쳐서 한 번에 전달
    if base_attn_mask is None:
        attn_mask_video = bias_mask  # [1,1,1,K]
    else:
        # 안전하게 결합: base_attn_mask 가 [B,1,1,K] 또는 [1,1,1,K] 등 "작은" 모양일 때만 더하기
        small_mask_shapes = (
            base_attn_mask.dim() == 4
            and base_attn_mask.size(-1) == K
            and base_attn_mask.size(-2) in (1,)  # Q축이 1인 마스크(키 패딩/전역 bias)
        )
        if small_mask_shapes:
            attn_mask_video = base_attn_mask + bias_mask  # 브로드캐스트로 안전 결합
        else:
            # 경고: 거대한 [B,H,Q,K] 마스크라면 이 합은 큰 텐서를 만들 수 있음
            # 가능하면 base 마스크를 key-패딩 형태([B,1,1,K])로 유지하세요.
            attn_mask_video = base_attn_mask  # 최후 수단: 바이어스 미적용 또는 호출부에서 분리 관리
            # 만약 꼭 합치고 싶다면, 호출부에서 base 마스크도 축소 형태로 재구성 권장

    out_video = F.scaled_dot_product_attention(
        q_video, key, value,
        attn_mask=attn_mask_video,
        dropout_p=dropout_p,
        is_causal=is_causal
    )

    # 6) 다시 붙여서 반환
    hidden_states = torch.cat([out_text, out_video], dim=-2)  # [B,H,Q,D]
    return hidden_states
...
def custom_scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    value_out = torch.zeros_like(value)

    for idx in range(math.ceil(L // chunk_size)+1):
        start = idx * chunk_size
        end = (idx + 1) * chunk_size

        query_chunk = query[:, :, start:end, :]

        atten_weight_chunk = query_chunk @ key.transpose(-2, -1) * scale_factor
        # atten_weight_chunk += attn_bias[start:end, :]
        atten_weight_chunk = torch.softmax(atten_weight_chunk, dim=-1)
        # atten_weight_chunk = torch.dropout(atten_weight_chunk, dropout_p, train=True)

        value_chunk = atten_weight_chunk @ value

        value_out[:, :, start:end, :] = value_chunk

    return value_out

class CogVideoXAttnProcessor2_0_attnW:
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """

    def __init__(self, store_list1=None, store_list2=None, store_list3=None, head_average=True):
        self.store_list1 = store_list1
        self.store_list2 = store_list2
        self.store_list3 = store_list3
    ...
    def __call__(
        ...
        if attn.norm_q is not None:
            query = attn.norm_q(query) # layer norm 
        if attn.norm_k is not None:
            key = attn.norm_k(key)  # layer norm
        ...
        scale = getattr(attn, "scale")

        q_img = query[:, :, text_seq_length:, :]
        k_txt = key[:, :, :text_seq_length, :] 

        q_txt = query[:, :, :text_seq_length, :]
        k_img = key[:, :, text_seq_length:, :] 

        scale = getattr(attn, "scale", 1.0 / (k_txt.shape[-1] ** 0.5))

        if self.steps == 10:
            pass

        scores_it = torch.matmul(q_img, k_txt.transpose(-2, -1)) * scale # it = img-txt
        scores_ti = torch.matmul(q_txt, k_img.transpose(-2, -1)) * scale # ti = txt-img 

        probs_it = F.softmax(scores_it, dim=-1) # torch.Size([2, 48, 17550, 226])
        probs_it_mh = probs_it.mean(1) # mh = mean head

        probs_ti = torch.softmax(scores_ti, dim=-1)  # [B, H, N_txt, N_img]
        probs_ti_mh = probs_ti.mean(1) # mh = mean head

        if self.store_list1 is not None:
            if self.store_list2 is not None:
                self.store_list1.append(probs_it_mh.detach().cpu())
                self.store_list2.append(probs_ti_mh.detach().cpu())
        
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )

        attn_mask_temp = torch.zeros(query.shape[-2], query.shape[-2], device=query.device, dtype=query.dtype)
        bad_idx = [0,1,2]
        attn_mask_temp[bad_idx, bad_idx] = float("-inf")
        
        hidden_states_bad = F.scaled_dot_product_attention(
            query[1:], key[1:], value[1:], attn_mask=attn_mask_temp, dropout_p=0.0, is_causal=False
        )

        diff_sumch_sumh = (hidden_states[1:] - hidden_states_bad).abs().sum(1).sum(-1)

        if self.store_list1 is not None:
            if self.store_list2 is not None:
                if self.store_list3 is not None:
                    self.store_list1.append(probs_it_mh.detach().cpu())
                    self.store_list2.append(probs_ti_mh.detach().cpu())
                    self.store_list3.append(diff_sumch_sumh.detach().cpu())
...
class CogVideoXAttnProcessor2_0_attnW_Grad:
    r"""
    Processor for implementing scaled dot-product attention for the CogVideoX model. It applies a rotary embedding on
    query and key vectors, but does not include spatial normalization.
    """

    def __init__(self, store_list=None, head_average=True):
    ...
    def __call__(
        ...
        # --- 텍스트/비디오 블록 분리 ---
        q_img = query[:, :, text_seq_length:, :]            # [B,H,N_img,Dh]
        k_txt = key[:, :, :self.token_len, :]              # [B,H,N_txt,Dh]
        v_txt = value[:, :, :self.token_len, :]            # [B,H,N_txt,Dh]   # ★ 추가

        scale = getattr(attn, "scale", 1.0 / (k_txt.shape[-1] ** 0.5))

        q_img_det = q_img.detach()
        k_txt_det = k_txt.detach()
        v_txt_det = v_txt.detach()   # v_txt는 grad 필요 없음

        with torch.cuda.amp.autocast(enabled=False):     # ⬅️ AMP 끄고
            with torch.no_grad():
                attn_scores = torch.matmul(q_img_det, k_txt_det.transpose(-2, -1)) * scale 
            attn_scores.requires_grad_(True)  # leaf로 만듦
            probs = F.softmax(attn_scores, dim=-1) # torch.Size([2, 48, 17550, 226])

        if attention_mask is not None:
            # (필요하면 텍스트 파트에 맞춰 마스크를 잘라 더해도 됨)
            pass

        # --- 텍스트 키에 의한 출력 구성 (grad flow 대상) ---
        out_txt = torch.matmul(probs, v_txt_det)                                  # [B,H,N_img,Dh]  # ★ 추가
        out_txt = out_txt.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)  # [B,N_img,C]
        ...
        out_txt_proj = F.linear(
                            out_txt,                                  # [B,N_img,C]
                            attn.to_out[0].weight.detach(),
                            attn.to_out[0].bias.detach()
                        )

        hs_text = hidden_states_proj[:, :text_seq_length, :]          # [B, 226, C]
        hs_img  = hidden_states_proj[:, text_seq_length:, :]          # [B, 17550, C]

        hs_img = hs_img + (out_txt_proj - out_txt_proj.detach())      # [B, 17550, C]
        hidden_states = torch.cat([hs_text, hs_img], dim=1)           # [B, 17776, C]
        ...
        if not hasattr(attn, "_gw_scores"): attn._gw_scores = []
        if not hasattr(attn, "_gw_probs"):  attn._gw_probs  = []
        attn._gw_scores.append(attn_scores)       # [B,H,N_img,N_txt] (det leaf 기반)
        attn._gw_probs.append(probs.detach())     # 메모리 줄이려 detach
