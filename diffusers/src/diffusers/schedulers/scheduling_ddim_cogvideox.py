...
class CogVideoXDDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        prev_timestep = timestep - self.config.num_train_timesteps // self.num_inference_steps # e.g 현재 999, 다음(=prev) 979 

        # 2. compute alphas, betas
        alpha_prod_t = self.alphas_cumprod[timestep]
        alpha_prod_t_prev = self.alphas_cumprod[prev_timestep] if prev_timestep >= 0 else self.final_alpha_cumprod
        # alpha는 999일때 0, 0으로갈수록 증가함
        ...
        if self.config.prediction_type == "epsilon":
            ...
            pred_original_sample = (alpha_prod_t**0.5) * sample - (beta_prod_t**0.5) * model_output # model output은 model이 예측한것, sample은 update전의 latent
